...
SSH is on, the login user is kept as you configured it,
...
2. Open `network-config` and enter you Wi-Fi credentials and any modifications you want to make to the network config.
   The address defaults work for a home network on 192.168.0.0/24. If your router uses something else (for example 192.168.1.0/24), set `GATEWAY_HOME_LAN` to match, and make sure `GATEWAY_ETH_NET` does not overlap it.
3. Optional: Add DHCP reservations in `stage-gateway/01-network/files/dhcp-reservations.conf`
...
- `qemu-user-static` on Linux (`sudo apt install qemu-user-static`)
...
#### The easy way (although I've never actually tried it :/):
...
2. Insert the micro SD card (through an SD or USB adapter is fine)
...
1. Insert the card and power the Pi on. It boots, joins your WiFi, and gets an
   ...
   and screen, log in, and run `nmtui` to pick a network. Alternativly, you can connect your PC/Laptop ethernet directly to the Pi's ethernet and ssh to it using `GATEWAY_ETH_IP`
2. Give the Pi a fixed WiFi address on your router:
   - Log in to the router
   - Find the Pi in the client list. Note its MAC address and current IP
   - Find the DHCP settings, and add a reservation for that MAC. Easiest is to reserve the IP it already has.
3. Add a static route on your router:
   - Find the static/advanced routing page and add
     - Destination network: the value of `GATEWAY_ETH_NET` in your `network-config`
...
5. Plug your wired device into the Pi's Ethernet port. Several devices work too,
...
6. SSH into the Pi: `ssh <FIRST_USER_NAME>@<pi-wifi-ip>`. The login page lists
...
7. From your laptop, `ping <wired-device-ip>` and then `ssh` to it directly.
